...
// gocldr/number package. It returns cardinal and ordinal plural categories for a
// number in a given BCP-47 locale, matching JavaScript's Intl.PluralRules (and
// therefore fluent.js).
...
// gocldr/number.Options understood by number.PluralOperands. Plural selection
// must see exactly the digits the number formatter would display, so we forward
// the integer/fraction/significant-digit options verbatim and let gocldr/number
// apply the same resolution and rounding. Style/Currency are intentionally
// dropped:
